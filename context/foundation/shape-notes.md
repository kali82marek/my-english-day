...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "pain category"
      decision: "brakująca zdolność + dane uwięzione — sytuacje przepadają, nie ma narzędzia, które uczy z realnych przeżyć"
    - topic: "insight"
      decision: "nikt nie łączy dziennika sytuacji z automatycznym generowaniem fiszek"
    - topic: "persona scope"
      decision: "samouk angielskiego na poziomie podstawowym, uczący się z codziennych sytuacji życiowych"
    - topic: "auth strategy"
      decision: "email + hasło, konto w chmurze, płaski model bez ról"
    - topic: "mvp scope"
      decision: "ścięto: grafiki do fiszek, push notifications, dwuetapowa akceptacja. Zostawiono: transkrypcja głosowa, AI fiszki, algorytm powtórek"
    - topic: "primary success criterion"
      decision: "akceptacja fiszek AI ≥ 70% bez poprawek"
    - topic: "guardrails"
      decision: "nagrywanie błyskawiczne (sekundy), brak duplikatów fiszek"
  frs_drafted: 12
    - topic: "FR-003 input mode"
      decision: "tylko głos — to serce produktu, brak opcji tekstowej w MVP"
    - topic: "FR-004 transkrypcja"
      decision: "wysyłaj od razu, bez edycji transkrypcji — AI sobie poradzi z sensem"
    - topic: "FR-005 limit sytuacji"
      decision: "bez limitu — użytkownik sam reguluje ile nagrywa"
    - topic: "FR-006 typy fiszek"
      decision: "AI decyduje, które typy (słówka/zwroty/zdania) pasują do danej sytuacji"
    - topic: "FR-007 warianty"
      decision: "zostają — rozszerzenia ściśle w obrębie tej samej sytuacji, nie odpływanie"
    - topic: "FR-008 duplikaty"
      decision: "duplikat = to samo słowo/zwrot, nie synonimy — synonimy i warianty to różne fiszki"
    - topic: "FR-009 edycja fiszek"
      decision: "MVP: tylko akceptuj/odrzuć, bez edycji fiszek"
    - topic: "FR-011/012 algorytm powtórek"
      decision: "3 przyciski: Nie umiem / Prawie / Umiem"
...
- Produkt: aplikacja mobilna (natywna lub hybrydowa — decyzja downstream).
- Skala na start: mała (sam użytkownik + garstka osób).
- Przy skali v2 z grafikami: rozważyć współdzieloną bazę grafik fiszek między użytkownikami, żeby zaoszczędzić na generowaniu.
